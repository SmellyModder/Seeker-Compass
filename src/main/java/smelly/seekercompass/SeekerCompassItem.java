...
public class SeekerCompassItem extends Item {
	...
	private Entity getEntity(ServerWorld world, ItemStack stack) {
		return world.getEntityByUuid(NBTUtil.readUniqueId(stack.getTag().get("TrackingEntity")));
	}
	
	public static RotationData getRotationData(ItemStack stack) {
		...
		if(!world.isRemote && isNotBroken(stack)) {
			...
			if(tag != null && tag.contains("TrackingEntity")) {
				...
				if(tag.contains("Rotations")) {
					...
					double turn;
					if(stack.hasTag() && stack.getTag().contains("EntityStatus")) {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
		...
		if(isNotBroken(stack) && stack.hasTag() && stack.getTag().contains("TrackingEntity") && player.isSneaking()) {
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		if(isNotBroken(stack) && EnchantmentHelper.getEnchantmentLevel(SCEnchants.SUMMONING.get(), stack) > 0 && stack.hasTag() && stack.getTag().contains("TrackingEntity") && player.isSneaking()) {
			if(world instanceof ServerWorld) {
	...
	@Nullable
	public static EntityRayTraceResult rayTraceEntities(Entity p_221273_0_, Vector3d p_221273_1_, Vector3d p_221273_2_, AxisAlignedBB p_221273_3_, Predicate<Entity> p_221273_4_, double p_221273_5_) {
